...
import "./NewCustomerOfferPage.css";
import {
...
export default function NewCustomerOfferPage() {
  ...
  useEffect(() => {
    getAllNewCustomerOffers(adminToken, setOffers);
  }, [adminToken, changed]);
  ...
  const handleToggleStatus = async (id) => {
    const response = await toggleNewCustomerOffer(id, adminToken);
    if (response) {
  ...
  const handleDelete = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        const response = await deleteNewCustomerOffer(id, adminToken);
  ...
  return (
    <div className="new-customer-offer-page-container">
      <NewCustomerOffer
      ...
      <Box mt={4}>
        ...
        <Box className="new-customer-page-controls" display="flex" gap={2} mb={2}>
          <TextField
            label="Search"
            variant="outlined"
            size="small"
